...
//IF ok, return true
//else, return false
bool averaging_acce(bmi2_acce_value_t *acce_value, int size, bmi2_acce_value_t *return_avg)
{
	float avg = 0;
	float variance = 0;
	...
	for(int i=0;i<size;i++)
	{
		...
		return_avg->acce_x += (acce_value + i)->acce_x;
		return_avg->acce_y += (acce_value + i)->acce_y;
	}
	avg = avg/(float)size;
	return_avg->acce_x /=     size;
	return_avg->acce_y /=     size;
